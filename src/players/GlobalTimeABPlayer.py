...
class Player(AbstractPlayer):
    def __init__(self, game_time, penalty_score):
        ...
        self.my_score = 0
        self.rival_score = 0
        self.AlphaBeta = AlphaBeta(self.utility_function, self.succ_generator,
    ...
    def set_game_params(self, board):
        ...
        self.rival_score = 0
        self.find_fruits()
        #self.empty_spots= self.calculate_empty()
    ...
    def make_move(self, time_limit, players_score):
        ...
        time_limit= self.game_time/2
        curr_state = State(self.board, self.position,
                           self.my_score, self.rival_position, self.rival_score, True)
        i = 1
        score, operator = self.AlphaBeta.search(curr_state, i, True, float('-inf'), float('inf'))
        end_time = time.time()
        ...
        self.position = pos
        self.update_fruits({})
        self.game_time= self.game_time - (time.time()-start_time)
        return operator
